...
success: function(data) {
    ...
    for (var i=0; i<data.length; i++) {
        ...
        var row = $('<tr><td><b><img src="avatar.jpg" width="30px" height="30px" />' + data[i].username + ' :<i> '+ data[i].date + ':</i></b></br><p style="padding-left:80px">' + data[i].post + '</br><a data-toggle="modal" data-id="'+ commentId +'" title="Add this item" class="open-ReplyModal" href="#ReplyModal">Reply</a>'+'</p></td></tr>');
        $('#record').append(row);
        for (var r = 0; (r < data.length); r++)
                {
                    if ( data[r].parent_comment == commentId)
                    {
                        var comments = $('<tr><td style="padding-left:80px"><b><img src="avatar.jpg" width="30px" height="30px" />' + data[r].username + ' :<i> ' + data[r].date + ':</i></b></br><p style="padding-left:40px">'+ data[r].post +'</p></td></tr>');
...
$(document).ready(function() {
	$('#butsave').on('click', function() {
		...
		var _username = document.forms["frm"]["username"].value;
...
$(document).ready(function() {
	$('#btnreply').on('click', function() {
		...
		var _username = document.forms["frm1"]["Rname"].value;
